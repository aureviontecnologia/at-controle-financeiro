...
import { accountSpendableCents, totalReserved, totalSpendable, totalTicketBalance } from '@/lib/finance';
...
export default function AccountsScreen() {
  ...
  const regularAccounts = finance.accounts.filter((item) => item.type !== 'ticket');
  const ticketAccounts = finance.accounts.filter((item) => item.type === 'ticket');
  ...
  async function save() {
    ...
    setFeedback(null);
    if (cleanName.length < 2) return reportError('Nome da conta', 'Digite um nome com pelo menos 2 caracteres.');
    if (cleanInstitution.length < 2) return reportError('Instituição', 'Digite o banco ou a carteira utilizada.');
    const reloadDay = Number(expectedReloadDay);
    if (accountType === 'ticket' && (reloadDay < 1 || reloadDay > 31 || expectedReloadCents <= 0)) return reportError('Recarga do ticket', 'Informe o dia mensal entre 1 e 31 e o valor esperado da recarga.');
    const editingAccount = finance.accounts.find((item) => item.id === editingAccountId);
    if (editingAccount && balanceCents < (editingAccount.reservedCents ?? 0)) return reportError('Saldo protegido', `Há ${formatMoney(editingAccount.reservedCents ?? 0)} guardados em cofres. Retire do cofre antes de reduzir o total abaixo desse valor.`);
    if (!user) return;
    setSaving(true);
    try {
      if (user.demo) {
        ...
        else await createOnlineAccount({ householdId: finance.householdId, userId: user.id, name: cleanName, institution: cleanInstitution, type: accountType, openingBalanceCents: balanceCents, expectedReloadDay: accountType === 'ticket' ? reloadDay : undefined, expectedReloadCents: accountType === 'ticket' ? expectedReloadCents : undefined, idempotencyKey: createRequestKey.current });
      }
      setFeedback({ tone: 'success', text: `${editingAccountId ? 'Conta atualizada' : 'Conta criada'} e compartilhada com a família.` });
      resetForm();
      if (!user.demo) void finance.refresh();
    } catch (reason) {
      reportError(editingAccountId ? 'Conta não atualizada' : 'Conta não adicionada', reason instanceof Error ? reason.message : 'Tente novamente.');
    ...
    <KeyboardAvoidingView style={[styles.flex, { backgroundColor: palette.ink }]} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      <Screen>
        ...
        {!adding ? <>
          ...
          <Pressable accessibilityRole="button" accessibilityLabel="Gerenciar cofres e caixinhas" onPress={() => regularAccounts.length ? router.push('/account-details' as Href) : startCreate()} style={({ pressed }) => [styles.potShortcut, { backgroundColor: palette.mintDeep, borderColor: palette.mint }, pressed && styles.rowPressed]}><View style={[styles.icon, { backgroundColor: palette.surface }]}><PiggyBank size={20} color={palette.mint} /></View><View style={styles.copy}><AppText variant="section">Cofres e caixinhas</AppText><AppText variant="caption">{regularAccounts.length ? `${formatMoney(totalReserved(regularAccounts))} separados do saldo livre · toque para criar ou movimentar` : 'Crie uma conta comum e depois separe o dinheiro guardado'}</AppText></View><Plus size={20} color={palette.mint} /></Pressable>
          <Surface style={styles.list}>{regularAccounts.length ? regularAccounts.map((item, index) => <View key={item.id}>{index ? <Divider /> : null}<Pressable accessibilityRole="button" accessibilityLabel={`Editar ${item.name}`} onPress={() => startEdit(item)} style={({ pressed }) => [styles.row, pressed && styles.rowPressed]}><View style={[styles.icon, { backgroundColor: item.type === 'cash' ? palette.amberDeep : palette.mintDeep }]}>{item.type === 'cash' ? <Banknote size={19} color={palette.amber} /> : item.type === 'wallet' ? <WalletCards size={19} color={palette.mint} /> : <Landmark size={19} color={palette.mint} />}</View><View style={styles.copy}><AppText variant="body">{item.name}</AppText><AppText variant="caption">{item.institution} · {item.reservedCents ? `${formatMoney(item.reservedCents)} guardados` : 'toque para editar'}</AppText></View><AppText variant="mono" style={styles.money}>{formatMoney(accountSpendableCents(item))}</AppText></Pressable></View>) : <EmptyState title="Nenhuma conta comum" description="Adicione conta bancária, carteira digital ou dinheiro em espécie." />}</Surface>
          {ticketAccounts.length ? <><View style={styles.ticketHeading}><View><AppText variant="label">SALDO EM TICKET</AppText><AppText variant="section">{formatMoney(totalTicketBalance(ticketAccounts))}</AppText></View><AppText variant="caption" style={styles.ticketExplanation}>Separado do patrimônio e não cobre faturas.</AppText></View><Surface style={styles.list}>{ticketAccounts.map((item, index) => <View key={item.id}>{index ? <Divider /> : null}<Pressable accessibilityRole="button" accessibilityLabel={`Editar ${item.name}`} onPress={() => startEdit(item)} style={({ pressed }) => [styles.row, pressed && styles.rowPressed]}><View style={[styles.icon, { backgroundColor: palette.amberDeep }]}><Ticket size={19} color={palette.amber} /></View><View style={styles.copy}><AppText variant="body">{item.name}</AppText><AppText variant="caption">{item.expectedReloadDay && item.expectedReloadCents ? `${item.institution} · recarga de ${formatMoney(item.expectedReloadCents)} todo dia ${item.expectedReloadDay}` : item.institution}</AppText></View><AppText variant="mono" style={styles.money}>{formatMoney(item.balanceCents)}</AppText></Pressable></View>)}</Surface></> : null}
...
const styles = StyleSheet.create({
  flex: { flex: 1 }, header: { paddingTop: spacing.md, minHeight: 48, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, close: { width: 44, height: 44, borderRadius: 15, alignItems: 'center', justifyContent: 'center' }, feedback: { borderRadius: radii.md, padding: spacing.md },
  summary: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, add: { width: 46, height: 46, borderRadius: 15, alignItems: 'center', justifyContent: 'center' }, potShortcut: { minHeight: 86, borderRadius: radii.lg, borderWidth: StyleSheet.hairlineWidth, padding: spacing.lg, flexDirection: 'row', alignItems: 'center', gap: spacing.md }, ticketHeading: { flexDirection: 'row', alignItems: 'flex-end', justifyContent: 'space-between', gap: spacing.md }, ticketExplanation: { flex: 1, textAlign: 'right' }, list: { paddingVertical: spacing.xs }, row: { minHeight: 72, flexDirection: 'row', alignItems: 'center', gap: spacing.md, borderRadius: radii.sm }, rowPressed: { opacity: 0.62 }, icon: { width: 42, height: 42, borderRadius: 14, alignItems: 'center', justifyContent: 'center' }, copy: { flex: 1, minWidth: 0, gap: 2 }, money: { fontSize: 13, flexShrink: 0 }, notice: { borderRadius: radii.md, padding: spacing.md, gap: 3 },
