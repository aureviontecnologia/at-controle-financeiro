...
export default function ScheduledExpenseScreen() {
  ...
  const storedMethod = paymentMethods.some((item) => item.id === schedule?.paymentMethod) ? schedule?.paymentMethod as PaymentMethodId : schedule?.defaultCardId ? 'credit_card' : 'pix';
    ...
    try {
      if (paying && schedule) {
        ...
        if (user.demo) {
          ...
          await payOnlineScheduledExpense({ householdId: finance.householdId, scheduleId: schedule.id, sourceId: selectedSource.id, sourceKind: selectedSource.sourceKind, paymentMethod: method, paymentMethodDetail: detail, idempotencyKey: requestKey.current });
        }
      } else if (user.demo) {
