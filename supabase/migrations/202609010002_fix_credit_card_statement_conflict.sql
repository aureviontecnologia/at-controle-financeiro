-- Evita que o alvo do UPSERT seja resolvido como variavel PL/pgSQL.
-- A restricao nomeada torna o cadastro de cartao deterministico no Postgres.
...
begin
  ...
  if current_invoice > 0 then
    as_of_date := (now() at time zone 'America/Sao_Paulo')::date;
    current_due_month := date_trunc('month', as_of_date)::date;
    invoice_due := current_due_month
      + (least(card_due_day, extract(day from (current_due_month + interval '1 month - 1 day'))::int) - 1);
    ...
    if card_due_day <= card_closing_day then
      current_close_month := (current_due_month - interval '1 month')::date;
    else
      current_close_month := current_due_month;
    end if;
    invoice_close := date_trunc('month', current_close_month)::date
      + (least(card_closing_day, extract(day from (date_trunc('month', current_close_month) + interval '1 month - 1 day'))::int) - 1);
    previous_close_month := (date_trunc('month', invoice_close) - interval '1 month')::date;
    previous_close := previous_close_month
      + (least(card_closing_day, extract(day from (previous_close_month + interval '1 month - 1 day'))::int) - 1);
    insert into public.card_statements(
      ...
      target_household, created_card_id, previous_close + 1,
      invoice_close, invoice_close, invoice_due, current_invoice
    );
  end if;
  ...
  loop
    ...
    insert into public.card_statements(
      ...
      previous_close + 1, invoice_close,
